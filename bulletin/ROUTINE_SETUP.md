...
sends it in beehiiv. Skip cleanly, and say so in the release-comms channel, if
any of these hold: the previous issue is still an unsent draft; the changelog
has not been updated in more than 8 days; there is not enough new material. If
you cannot find bulletin/PROMPT.md, stop and report it there rather than
...
**The routine has never run end to end.** Issues #1 and #2 were both drafted
collaboratively in a session, not by it. Its first pass is the first real test —
watch the session log.

As of 2026-09-23, expect that first pass to **skip**: issue #2 is still an unsent
draft, so Step 0a stops it, and the ledger has **zero** `pending` rows anyway.
That is the correct behaviour, not a failure. It will have something to do once
the weekly routine has appended a few rows and #2 has gone out.
